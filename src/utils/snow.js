...
window.snow = (function () {
    ...
    function resetScreen(zIndex) {
        if (!canvas) {
            window.addEventListener("resize", () => resetScreen(zIndex));
        ...
        canvas.style.zIndex = zIndex || 5000;
        canvas.style.left = 0;
    ...
    function isDateInMonthDayRange(date, range) {
        ...
        if (startMonth <= endMonth) {
            ...
            // Range spans across year end (e.g., 11/15 - 01/15)
    ...
    return {
        start: function (options) {
            ...
            if (!options.forceEnable && options.monthDayRange) {
                if (!isDateInMonthDayRange(new Date(), options.monthDayRange)) {
                    return;
                }
            }
            if (!ctx) resetScreen(options.zIndex);
